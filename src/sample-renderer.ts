...
const INDI_MIN_HEIGHT = 100;
const INDI_MIN_WIDTH = 120;
const FAM_MIN_HEIGHT = 15;
const FAM_MIN_WIDTH = 80;
...
export function getLength3(text: string, textClass: string): number {
 ...
 * and death.
 */
export class SampleRenderer extends CompositeRenderer implements Renderer {
  ...
  getPreferredIndiSize(id: string): [number, number] {
    ...
    const maxDetailsWidth = max(
      details.map((x) => getLength3(x.text, 'details'))
    )!;
    const width =
      max([
        maxDetailsWidth + 22,
        getLength3(indi.getFirstName() || '', 'name') + 8,
        getLength3(indi.getLastName() || '', 'name') + 8,
        getLength3(id, 'id') + 32,
        INDI_MIN_WIDTH,
      ])! + ( 0);
    return [width, height];
